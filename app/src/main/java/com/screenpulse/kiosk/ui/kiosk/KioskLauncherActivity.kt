...
class KioskLauncherActivity : AppCompatActivity() {
    ...
    override fun onResume() {
        ...
        if (!hasRequiredPermissions()) {
            requestPermissions(REQUIRED_PERMISSIONS, 2001)
            return
        }
        
        // 2. Check Special Permissions (Overlay & Usage Stats)
        ...
        // 3. Check Wi-Fi Onboarding
        // If we are already connected, mark onboarding as done automatically
        if (isConnected) {
            if (!config.wifiOnboardingCompleted) {
                config.wifiOnboardingCompleted = true
                ConfigManager.saveConfig(this, config)
            }
        }
        
        // Only launch Captive Wifi if NOT completed AND NOT connected
        ...
        // 4. Handle Kiosk Mode / Setup State
        val setupButton = findViewById<android.widget.Button>(com.screenpulse.kiosk.R.id.setupButton)
        val loadingText = findViewById<android.widget.TextView>(com.screenpulse.kiosk.R.id.loadingText) // Need to add ID to XML
        val progressBar = findViewById<android.widget.ProgressBar>(com.screenpulse.kiosk.R.id.progressBar) // Need to add ID to XML

        if (KioskManager.isKioskEnabled(this)) {
            setupButton.visibility = View.GONE
            loadingText?.visibility = View.VISIBLE
            loadingText?.text = "Loading your menu screens..."
            ...
            setupButton.visibility = View.VISIBLE
            loadingText?.visibility = View.VISIBLE
            loadingText?.text = "Kiosk Mode Disabled\nTap below to Setup"
